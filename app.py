...
def db_connect():
    # Enable SQLite WAL for better concurrency
...
def today_in_window(bday: date, check: date | None = None) -> tuple[bool, date, date]:
    ...
    Returns (in_window, window_start, window_end).
    """
    if check is None:
...
async def complete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with closing(db_connect()) as conn:
        ...
        in_window, start, end = today_in_window(bday, today)
        cur = conn.cursor()
        # Mark complete for current year regardless of window
        cur.execute(
            """
            UPDATE users
...
• /import_csv  — send a CSV file (headers: personnel_id,birthday)
...
async def import_csv_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(msg.from_user.id):
        return await msg.reply_text("Admins only.")
    await msg.reply_text(
        "Send me a CSV file with headers: personnel_id,birthday (YYYY-MM-DD). I will parse and import it."
    )
...
async def import_csv_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc.file_name.lower().endswith(".csv"):
        return await update.message.reply_text("Please upload a .csv file.")
    ...
    with closing(db_connect()) as conn:
        ...
        required = {"personnel_id", "birthday"}
        if set(reader.fieldnames or []) != required:
            return await update.message.reply_text(
                "CSV must have exactly these headers: personnel_id,birthday"
            )
        for row in reader:
            pid = (row.get("personnel_id") or "").strip()
...
def setup_handlers(app: Application):
    ...
    app.add_handler(MessageHandler(filters.Document.FILE_EXTENSION("csv"), import_csv_file))

def schedule_jobs(app: Application):
    # Schedule daily reminder at 09:00 local time
    now_local = datetime.now(TZINFO).replace(hour=REMINDER_HOUR, minute=0, second=0, microsecond=0)
