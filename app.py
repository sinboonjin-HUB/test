...
def parse_birthday_any(val: Any) -> Optional[date]:
    ...
    if isinstance(val, (int, float)):
        # Rare: if Excel serial date leaks in (shouldn't with openpyxl), ignore
        return None
    s = str(val).strip()
    if not s:
        return None
...
def _extract_records_from_xlsx_bytes(b: bytes) -> Iterable[Tuple[str, str]]:
    ...
    for r in rows[1:]:
        ...
        # Parse DOB if it's a date-like cell
...
# App Bootstrap
# ----------------------
async def admin_help(update: Update, context: ContextTypes.DEFAULT_TYPE):  # already defined above
    pass  # placeholder to satisfy linter in this snippet (actual function is above)
