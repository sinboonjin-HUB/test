...
def next_birthday_after_year(bday: date, start_year: int) -> date:
    """Given a birthday and a start_year, return the next birthday date after start_year's birthday."""
    start = adjusted_birthday_for_year(bday, start_year)
    return adjusted_birthday_for_year(bday, start_year + 1)
...
def today_in_window(bday: date, check: date | None = None) -> tuple[bool, date, date]:
    """
    Full-year window: from the birthday in 'check.year' to the next birthday.
    Returns (in_window, start, end), where start is inclusive and end is the next birthday (exclusive).
    """
    if check is None:
        check = current_local_date()
    # Determine which year's birthday is the current window start
    this_year_bday = adjusted_birthday_for_year(bday, check.year)
    if check >= this_year_bday:
        start = this_year_bday
        end = next_birthday_after_year(bday, check.year)
    else:
        # We're before this year's birthday; window started last birthday
        start = adjusted_birthday_for_year(bday, check.year - 1)
        end = this_year_bday
...
• /status — view your current window & status (birthday → next birthday)
• /complete — mark this window's IPPT as completed
• /uncomplete — undo your completion for the current window
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    window_key = start.year  # track completion by window start year
    ...
    lines = [
        ...
        f"Window: <b>{format_date(start)}</b> → <b>{format_date(end)}</b> (next birthday)",
        f"Today: <b>{format_date(today)}</b> — {'✅ In window' if in_window else '🕒 Outside window'}",
        f"Status (window {window_key}): {'✅ Completed' if done_this_window else '❌ Not completed'}",
        f"Deferment: {defer_text}",
        f"Reminder interval: every <b>{REMINDER_INTERVAL_DAYS}</b> days",
    ]
    if not done_this_window:
        if nrd:
...
async def complete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text(
        f"✅ Recorded as completed for window starting {window_key}. No more reminders this window.\n"
        f"(Window: {format_date(start)} → {format_date(end)}.)"
    )


async def uncomplete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Users: undo your completion for the current window."""
    msg = update.message
    today = current_local_date()
    with closing(db_connect()) as conn:
        data = get_personnel_and_user(conn, msg.from_user.id)
        ...
        conn.commit()
    if cleared:
        await msg.reply_text(f"↩️ Your completion for window {window_key} has been undone. You'll resume reminders if in window.")
    else:
        await msg.reply_text(f"No completion found to undo for window {window_key}.")
...
• /admin_uncomplete <tokens> [WINDOW_START_YEAR] — undo completion by Telegram ID or personnel_id
• /defer_reason  <tokens> [WINDOW_START_YEAR] -- <reason text>

Note: The IPPT window is from the member's current birthday to their next birthday.
        """.strip()
    )
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Also include days_left (to next birthday) or days_overdue (past next birthday).
    \"\"\"
    ...
    for pid, bday_str, group_name, telegram_id, completed_year, completed_at in rows:
        ...
        # Deferment fetch for this window
        d_status = ""
        d_reason = ""
        if verified:
            with closing(db_connect()) as conn:
    ...
    caption = (
        f"Window report (birthday → next birthday)\n"
        f"Completed: {completed_count}\n"
        f"Outstanding: {outstanding_count}\n"
        f"Red rows: not completed and no approved deferment"
    )
    await msg.reply_document(
        document=InputFile(out, filename=f"ippt_window_report.xlsx"),
        caption=caption,
    )

# ----------------------
# Token resolution & Admin ops (unlink/remove/uncomplete/deferment)
# ----------------------
...
async def admin_uncomplete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if year is None:
        # Infer current window key for today
        # Choose an arbitrary birthday (Feb 28) to compute; better approach is per user below
        pass
    ...
    with closing(db_connect()) as conn:
        cur = conn.cursor()
        for tid in tids:
            # Determine this user's current window key if year not provided
            target_year = year
            if target_year is None:
                ...
                _, start, end = today_in_window(bday, current_local_date())
...
async def daily_reminder_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for telegram_id, bday_str, completed_year in rows:
        try:
            ...
            # Skip if has approved deferment for this window
            skip = False
            with closing(db_connect()) as conn:
                d = get_deferment(conn, telegram_id, window_key)
...
def setup_handlers(app: Application):
    ...
    app.add_handler(CommandHandler("admin_uncomplete", admin_uncomplete))app.add_handler(CommandHandler("defer_reason", defer_reason))
