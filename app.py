...
• /unlink_user <TELEGRAM_ID> — remove a Telegram user's link & completions
• /remove_personnel <PERSONNEL_ID> — remove personnel + linked users & completions
    ...
    with closing(db_connect()) as conn:
        ...
        for pid, bday in records:
            ...
            try:
                ...
                skipped += 1
                continue
            # Update first to categorize; if none updated, insert
...
async def unlink_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) != 2:
        return await update.message.reply_text("Usage: /unlink_user <TELEGRAM_ID>")
    try:
        tid = int(parts[1])
    except ValueError:
        return await update.message.reply_text("TELEGRAM_ID must be a number (get it via /whoami).")
    ...
    if deleted_users:
        return await update.message.reply_text(f"🧹 Unlinked user {tid}. Deleted {deleted_completions} completion row(s).")
    else:
        return await update.message.reply_text("No such Telegram user ID found.")
...
async def remove_personnel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parts = update.message.text.split()
    if len(parts) != 2:
        return await update.message.reply_text("Usage: /remove_personnel <PERSONNEL_ID>")
    pid = parts[1].strip()
    ...
    with closing(db_connect()) as conn:
        ...
        deleted_personnel = cur.rowcount

        conn.commit()

    if deleted_personnel:
        return await update.message.reply_text(
            f"🗑️ Removed personnel {pid}. Unlinked {deleted_users} user(s), deleted {deleted_completions} completion row(s)."
        )
    else:
        return await update.message.reply_text("No such personnel_id found.")
