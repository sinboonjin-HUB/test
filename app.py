...
def next_birthday_after_year(bday: date, start_year: int) -> date:
    """Return the next birthday date after the birthday in start_year."""
...
• /defer <reason> — add an active deferment reason (pauses reminders)
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    window_key = start.year  # completion is tracked by window start year
...
async def defer_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with closing(db_connect()) as conn:
        ...
        conn.commit()
    await msg.reply_text(f"📝 Deferment active for window {window_key}.\nReason: {reason}")
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_document(
        document=InputFile(out, filename="ippt_window_report.xlsx"),
        caption=caption,
    )
...
async def admin_uncomplete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with closing(db_connect()) as conn:
        cur = conn.cursor()
        for tid in tids:
            # Determine this user's window key if year not provided
            target_year = year
